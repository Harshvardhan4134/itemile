...
const ForgotPassword: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEmail(input)) {
      ...
      try {
        ...
        await signInWithPhoneNumber(auth, phoneWithCountry, recaptchaVerifier);
        setOtpSent(true);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setOtpLoading(true);
    try {
      // For Firebase, we'll use a simpler approach
      // In a real app, you'd implement proper phone auth flow
